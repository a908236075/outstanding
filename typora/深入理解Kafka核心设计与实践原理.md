...
2. 分区中有主副本(leader)和从副本(follow)组成(一主多从).
   ...
   - **问题：为什么 Kafka 默认不支持“主写从读”（读写分离）？**
   
     很多数据库（如 MySQL）和缓存（如 Redis）都采用读写分离来提升性能，但 Kafka 默认不这么做，主要基于以下考虑：
   
     1. **避免数据不一致：** 主从同步存在网络延迟。如果消费者从 Follower 读取，可能会读不到 Leader 刚刚写入的数据（幻读），增加了系统复杂度。
     2. **规避主从同步延迟带来的负载均衡问题：** 消费者如果是根据 Offset 消费的，主从延迟会导致消费者在切换拉取节点时，处理逻辑变得非常复杂。
     3. **Kafka 的瓶颈通常不在单节点读写：** Kafka 的数据是顺序追加写磁盘，利用了操作系统的 Page Cache 和零拷贝（Zero-copy）技术，单机读写性能极高。通过**增加分区数（Partition）**并将主副本分散到不同的 Broker 上，就已经能很好地实现水平扩展和负载均衡，无需强求分担单点读压力的读写分离。
...
4. 消费模式有推模式和拉模式两种.一般使用拉取的模式,牺牲了实时性,但是提升了性能.(消费者主动去拉).
...
10. seek()有指定分区位置消费的方法,还可以指定时间,消费到昨天或者之前的数据.执行之前需要执行poll()方法作为前置的条件.
    ...
    1. 再均衡是指分区的所属权从一个消费者转移到另一个消费者的行为.消费组会添加或者删除一个成员,在均衡发生期间,消费组内的消费者是无法读取消息的.
    ...
    3. 调用ConsumerRebalanceListener,重写方法,在均衡器触发之前和之后进行相应的操作.

11. 消费者拦截器:在poll消息抵达之前(onConsume)或之后(onCommit)进行一些操作.
...
1. 协议设计:TCP的二进制协议.每种协议类型请求头和返回头都包含一个correlation_id,客户端的标识,ProduceRequest和FetchRequest协议结构,想看翻书吧.
2. 时间轮:存储定时任务的环形队列,底层采用数组实现,数组中每一个元素可以存放一个定时任务列表.
3. 时间轮对于定时任务的添加和删除复杂度为O(1),性能高出DelayQueue.但是kafka在推进时间轮的时候,使用了DelayQueue,避免了推进任务时候的**空推进**.例如一个定时任务在200s之后,不用按照时间间隔一个一个的推进,而是使用DelayQueue直接进行推进.
4. kafka多个Broker会选举一个**控制器**,负责分区leader副本的选举工作.监控分区相关的变化,监听主题相关的变化,监听broker相关的变化.
5. 分区leader副本的选举:一般是从ISR集合中进行选举.
...
3. 在均衡原理:
   ...
   2. 第二阶段:JOIN_GROUP:消费者会向GroupCoordinator发送JoinGroupRequest请求,并处理响应.GroupCoordinator主要做两件事:
      ...
      2. 选举分区分配策略:少数服从多数为原则.并不是有Leader决定的.
...
8. broker服务端维护为每一个生产者(<pid,分区>)维护一个版本号,通过比较版本号实现幂等的功能.**pid**为每一个新的生产者生成的.**幂等只能保障一个分区消息不重复写入.**
9. 事务:可以保证多个分区的**写入操作**的原子性.
...
1. 日志量:20000条/s,一条日志大约为1k左右,20M/s.均值为1000条/s及1M/s,一天12小时 产生50G左右日志.3个副本保存3天算上富余量1T左右
2.  硬盘1T,服务器3台,千兆网卡
3. cup 负责写磁盘线程8,副本拉取线程1,数据传输3个线程.
