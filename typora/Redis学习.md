...
3. 基础数据结构:String,list,hash,set,zset.
   ...
   6. hyperloglog:存入的数据量非常大,但是基数较少的数据.去重复,不直接存储数据本身,通过牺牲准确性来换取空间.
      ...
      1. ~~~shell
         ...
         # 记录用户 ID
         PFADD page_uv user_1 user_2 user_3 user_1
         # 获取结果
         PFCOUNT page_uv  # 返回 3
         ~~~
         
      2. 应用场景
         1. 主要是统计日活数,将日期作为键,ip作为值,直接count获得答案.
         1. **网站唯一访客（UV）统计**：统计一天内访问某个页面的独立用户数（即便同一个用户访问 100 次，也只计为 1）。
...
1. ### 简单动态字符串(SDS):
   ...
   2. 与C语言字符串的比较:
      ...
      - **如果修改后 SDS 的长度（`len`）小于 1MB：**
      
        Redis 会分配与 `len` **同样大小**的未使用空间。
      
        > **公式：** 程序实际分配的总长度 = `len` + `len` + 1 byte (用于 `\0`)
        >
        > **你的例子：** 13 字节字符串，分配 13 字节未使用空间，加 1 字节结束符，总共 $13 + 13 + 1 = 27$ 字节。
      
        **如果修改后 SDS 的长度（`len`）大于或等于 1MB：**
      
        为了防止空间浪费，Redis 不再翻倍分配，而是**固定分配 1MB** 的未使用空间。
      
        > **公式：** 程序实际分配的总长度 = `len` + 1MB + 1 byte
        >
        > **例子：** 如果修改后是 10MB，总分配就是 11MB + 1 byte。
...
4. ### 跳跃表
   ...
   1. ZSet底层实现,用空间换取时间.实现有序集合键和集群节点中做内部数据结构.
   ...
   4. ![image-20260310141713071](picture/Redis学习/image-20260310141713071.png)
...
2. ### 集群模式
   ...
   4. **槽指派**:一共有16383个槽,通过命令 CLUSTER ADDSLOTS <slot> [slot ....]指派.节点会将自己管理的槽广播出去,每个节点都会知道其他节点负责管理的槽.将他们保存在一个数组中.是虚拟层,主要是为了扩容.
   
   5. 为什么要使用哈希槽（Hash Slot）？首先做一个小纠正：Redis 集群的槽位总数是 **16384** 个，其编号范围是 `0` 到 `16383`。
   
      在处理包含千万级数据、且需要维持毫秒级极速响应的系统（例如 B2B 业务中的核心价格中心）时，单机 Redis 的内存和吞吐量迟早会遇到瓶颈。引入 Redis Cluster 进行横向扩展是必然的选择。
   
      哈希槽的本质，是在 **Key** 和 **Redis 物理节点** 之间增加的一层**抽象路由层（中间层）**。
   
      1. **解耦数据与节点，解决扩缩容痛点**
         - 如果使用传统的取模算法 *ha**s**h*(*k**ey*)(mod*N*)（N 为节点数），当集群增加或删除一个节点时，N 发生变化，会导致几乎所有 Key 的路由结果都发生改变，引发大面积的缓存失效（甚至缓存雪崩）。
         - 引入槽之后，计算公式变为：*H**A**S**H*_*S**L**OT*=*CRC*16(*k**ey*)(mod16384)。Key 固定的映射到某个具体的槽位，而槽位再分配给具体的物理节点。
      2. **实现平滑的数据迁移**
         - 当需要扩容增加节点时，只需将部分“槽”以及槽里的数据从老节点迁移到新节点即可。
         - 在迁移过程中，集群依然可以对外提供服务，未参与迁移的槽的数据完全不受影响。这对于不能容忍长时间停机维护的业务基座来说，是保证高可用性的关键设计。
   
   6. **命令执行**:对键进行hash,计算出它对应的槽,如果是是当前节点管理的槽,就直接执行,如果不是,就发送**moved错误**给客户端,根据返回的ip和端口号指引客户端将命令转移到对应的节点进行处理.
   
   7. **ASK错误**:当从新分配槽节点的时候,可能会出现一部分数据在目标的节点,一部分在源节点上,当数据在源节点没有找到的时候,就会发送ASK的错误给客户端.
   
   8. **ASKING命令**:当客户端携带ASKING标识的时候,会破例执行所对应的命令一次,而不是返回给客户端moved错误.????
   
   9. **MOVED错误和ASK错误的区别**:MOVED错误表示槽的负责权已经从一个节点转移到了另一个节点,而ASK错误只是两个节点在迁移槽的过程中使用的一种临时的措施.
...
1. redis利用epoll来实现多路复用,将连接信息和事件放入到队列中,一次放到文件事件分派器,分派器将事件发送给处理器.
...
4. LRU VS LFU
   ...
   1. 简单来说，**LRU** 关注的是**时间（多久没用了）**，而 **LFU** 关注的是**频率（用了多少次）**。
   2. **Redis 的近似 LRU**：为了节省内存，Redis 并没有使用链表（指针太占空间），而是给每个对象存了一个 **24 bits 的时钟（Lruclock）**。淘汰时，随机采样 $N$ 个 Key，删掉其中最久没更新的那个。
   3. **Redis 的 LFU 实现**：由于 24 bits 空间有限，Redis 将这 24 位拆成了两部分：
      1. **Last Decrement Time (16 bits)**：记录最后一次访问时间（用于衰减）。
      2. **Logistic Counter (8 bits)**：访问计数器。

# 常见面试题

## 7.0之后的新特性(Redis 7.4 - 8.6 )

### 一、 核心架构演进：从“缓存”到“多模态数据库”

**面试题：Redis 8.0 最大的变化是什么？**

- **回答要点：** **原生全功能集成（The Great Convergence）**。
- **内容：**
  - 以前需要单独安装模块的 **JSON、Search (全文搜索)、Vector (向量数据库)、TimeSeries** 现已全部**内置**。
  - **面试金句：** “Redis 8.0 不再只是一个 KV 缓存，它通过内置向量索引，可以直接支持 **RAG (检索增强生成)** 架构，作为 AI 应用的内存中心。”

------

### 二、 数据结构大更新：Hash Field TTL (7.4+)

**面试题：如果 Hash 里的某个字段要过期，以前怎么做？现在呢？**

- **旧方案：** 只能给整个 Key 设过期，或者把字段拆成独立的 String Key（浪费内存）。
- **新方案 (7.4+)：** 支持 **字段级过期 (Field TTL)**。
- **面试金句：** “使用 `HEXPIRE` 命令可以精准控制 Hash 内部字段的生命周期，这在**多属性 Session 管理**和**购物车**场景下，比以前省下了约 30% 的 Key 空间开销。”

------

### 三、 持久化黑科技：Multi-part AOF (7.0+)

**面试题：AOF 重写（Rewrite）时产生的性能抖动怎么解决？**

- **原理：** 放弃了单文件 AOF，改用 **Base文件 + 增量文件 (Incr)**。
- **面试金句：** “MP-AOF 彻底取消了重写期间的‘增量缓冲区’合并操作。它通过 **Manifest 文件** 管理多个 AOF 分片，让重写过程对主线程的影响几乎降为零，极大提升了长尾请求的稳定性（P99 延迟）。”

------

### 四、 极致内存优化：Redis 8.6 的元数据压缩

**面试题：Redis 8.6 在内存占用上做了哪些“榨取”？**

- **要点：**
  - **指针嵌入 (Pointer Embedding)**：小对象不再使用 64 位指针，直接在 `dictEntry` 内部存储。
  - **Listpack 深度化**：进一步扩大了 Listpack 的应用范围。
- **面试金句：** “8.6 版本通过对 `redisObject` 头的按位压缩和 `dictEntry` 的瘦身，实现了在不改变业务逻辑的前提下，**纯内存成本降低了 10%-15%**。这对于百 GB 级别的集群来说是巨大的降本增效。”

------

### 五、 避坑指南：许可证与 Valkey 分支

**面试题：听说 Redis 换协议了，对我们公司有什么影响？**

- **背景：** 7.4 开始变更为非严格开源协议（RSALv2/SSPLv1）。
- **对策：** 如果是云服务商或二次开发卖产品，会受限；普通企业内部使用无影响。
- **面试金句：** “虽然协议变了，但社区出现了 100% 兼容的 **Valkey**。作为架构师，我会持续关注底层 API 的兼容性，确保在必要时能无缝迁移，规避商业风险。”

------

### 六、 性能优化：IO 线程模型 (6.0 - 8.0)

**面试题：Redis 是单线程的，为什么还能撑起 10w+ QPS？**

- **回答逻辑：**
  1. **内核态**：非阻塞 IO 多路复用 (`epoll`)。
  2. **执行态**：单线程处理指令，避免锁竞争。
  3. **传输态 (6.0/8.0)**：**多线程 IO** 处理网络读写（数据搬运）。
- **面试金句：** “Redis 的‘单线程’是指 **命令执行逻辑**。而 8.0 进一步优化的多线程 IO，解决了大包传输时的 CPU 瓶颈，让 Redis 能够吞满万兆网卡的带宽。”

## Redis为什么这么快

1.  纯内存操作 (最根本原因)
2. I/O 多路复用模型 (Reactor 模式)
   1. Redis 处理高并发连接的秘密武器是 I/O 多路复用（如 Linux 下的 `epoll`，Mac 下的 `kqueue`）。 单线程并不意味着它只能同时处理一个客户端请求。通过 I/O 多路复用机制，Redis 的主线程可以同时监听成千上万个套接字（Socket）。当某个套接字准备好读取或写入时，系统才会通知 Redis 去处理，这就使得单线程能够高效地处理大量的并发网络连接，而无需阻塞等待。
3. 避免了多线程带来的额外开销
   1. 无上下文切换成本
   2. 无锁竞争
4. 高效且优化的底层数据结构

**Redis 的性能瓶颈通常是内存大小或网络带宽，而不是 CPU。**

**补充说明**：Redis 真的只有单线程吗？

其实随着版本迭代，Redis 早就不是严格意义上的单线程了：

- **Redis 4.0 引入了后台线程（Bio Threads）：** 专门用于处理一些耗时且不影响核心逻辑的操作，比如异步删除大对象 (`UNLINK`)、AOF 刷盘等，防止主线程被阻塞。
- **Redis 6.0 引入了多线程网络 I/O：** 这是最大的改变。随着网络硬件升级，网络 I/O 的读写开始成为瓶颈。Redis 6.0 引入了多个 I/O 线程来并发处理网络请求的接收、解析和回复。**但是，核心的命令执行阶段依然是单线程的。** 这保证了 Redis 依然不需要加锁，保留了其简洁和无锁的优势。

## IO多路复用

1. 只使用一个服务端进程可以同时处理多个套接字描述符连接.
2. 解决了**网络等待**的阻塞，但无法解决**业务执行**的阻塞。
3. select ->poll  -> epoll
   1. <img src="picture/Redis学习/image-20260312165354813.png" alt="image-20260312165354813" style="zoom:50%;" />
   2. <img src="picture/Redis学习/image-20260312165728630.png" alt="image-20260312165728630" style="zoom:50%;" />
   3. ![image-20260312165914266](picture/Redis学习/image-20260312165914266.png)
   4. <img src="picture/Redis学习/image-20260312170206923.png" alt="image-20260312170206923" style="zoom: 67%;" />

## 跳表的数据结构 与 红黑树的区别

1. ### 什么是连锁更新

   1. 为什么 ZipList 会有“连锁更新”？

      在 ZipList（压缩列表）中，每一个节点的头部都有一个字段叫 `prevrawlen`，它记录了**前一个节点的长度**。

      - **设计初衷**：为了支持从后往前遍历（双向遍历）。
      - **潜伏的危机**：`prevrawlen` 字段的大小是变长的：
        - 如果前一个节点长度 < 254 字节，它占 **1 字节**。
        - 如果前一个节点长度 > 254 字节，它会扩容到 **5 字节**。

      **这就是“多米诺骨牌”的起点：**

      假设你有连续 10 个节点，长度都在 250 到 253 字节之间。这时，如果你把第一个节点修改为 254 字节：

      1. 第二个节点的 `prevrawlen` 需要从 1 字节变成 5 字节。
      2. 导致第二个节点的总长度也超过了 254 字节。
      3. 第三个节点的 `prevrawlen` 也得跟着扩容...

      **结果**：一次简单的修改可能导致全表内存重分配，时间复杂度从 O(N)飙升到 O(N^2)，在极端情况下会导致 Redis 瞬间卡死。

2. ### 跳表是怎么解决链锁更新的

   1. **ZipList 的逻辑**：节点 B 存的是“节点 A 的长度”。如果要跳，先读 B 里的 `prevlen`，然后根据这个值往回跳。

      - *隐患*：一旦 A 的长度变了，B 的 `prevlen` 字段大小可能也要变（从 1 字节变 5 字节），引发连锁更新。

      **Listpack 的逻辑**：节点 B 存的是“节点 B 自己的长度”。

      - *优势*：A 变大变小，跟 B 没关系。B 永远只记录自己多长。只要 B 没动，它末尾的长度字段就不需要重分配。
      
      - 如果程序从后往前遍历，读到了当前元素的 `Backlen`（假设是 10 字节），程序就会拿着当前内存指针**直接往回跳 10 个字节**。这一跳，刚好就落在了当前元素的开头，也就是**上一个元素的末尾**。
      
        **收益极其巨大：** 每个元素只关心自己的大小，与前后的邻居彻底脱钩。不管你在前面插入多大的数据，后面的元素永远不需要修改自己的字段。彻底杀死了“连锁更新”引发的系统卡顿。

## 大Key怎么进行优化

1. 序列化与压缩：从源头“脱水” 开启压缩列表.

   1. **采用高效序列化协议**：抛弃占用空间较大的 JSON 文本，改用 Protobuf 或 Kryo 等二进制序列化框架。
   2. **开启压缩**：在 Java 端使用 GZIP、Snappy 或 LZ4 算法对数据进行压缩后再存入 Redis（String 类型），读取时在内存中解压。通常可以获得 60% - 80% 的体积缩减。

2. 拆分大 Key：化整为零（Sharding）

   这是最核心的优化。将一个含有 10 万个字段的 `BigHash` 拆分成多个 `SmallHash`。

   - **方案**：采用 **Fixed-size Sharding**。
     - 假设原本 Key 是 `price:matrix`，里面有 10000 个 Field。
     - 按照 Field 名做 Hash（或取模），拆分成 `price:matrix:1` 到 `price:matrix:100`。本来放在一起 现在将模数作为参数拆分成多个.

3. 需求 值保存0,1 用代码去查询.

## 双写一致性

1. 这是最基础、应用最广的策略。它的核心原则是：**读时更新，写时淘汰。**
   - **读请求：** 先查 Redis。命中则直接返回；未命中则查数据库，将结果回写到 Redis，然后返回。
   - **写请求：先更新数据库，后删除缓存。**
   - **进阶：延迟双删**
     - **步骤：** 先删除缓存 -> 更新数据库 -> **休眠短暂时间（例如 500ms）** -> 再次删除缓存。

## 缓存的击穿或者雪崩

### 击穿

1. 击穿:击穿通常发生在**单个极其热门的“热点 Key”\**上。\****,如果在某一个毫秒，这个 Key 的过期时间刚好到了（失效了），那么这几万个并发请求会瞬间“穿透” Redis.
   1. 方案 A：互斥锁 (Mutex Lock / 分布式锁)
   2. 方案 B：逻辑过期 (Logical Expiration / 异步重建):
      1. **思路：** 对于极其敏感的热点 Key，**物理上永远不设置 TTL（不让它在 Redis 里真正过期）**。而是在存入的 Value 对象中，附加一个“逻辑过期时间”字段。
      2. **执行：** 每次读取时，应用层判断这个逻辑时间是否已过。如果没过，直接返回；如果过了，**立刻返回这套旧的计价数据给前端**（保证可用性和极速响应），同时在后台异步开启一个单独的线程去 MySQL 拉取最新数据并更新 Redis。

### 雪崩

1. 大批量 Key 同时过期/Redis 节点宕机
2. 方案 A：过期时间打散 (TTL Jitter)
3. 方案 B：缓存预热 (Cache Pre-heating)
4. 方案 C：多级缓存兜底
5. 方案 D：终极防线 —— 限流降级

## JVM 级别的本地缓存（如 Caffeine） 怎么保证集群数据的同步

1. 佛系解法：短期 TTL（最终一致性）:给 Caffeine 设置一个极短的过期时间（例如 `expireAfterWrite = 5秒` 或 `1分钟`）。
2. 业界标配：基于 Pub/Sub 的主动失效通知
3. 解耦极客版：Canal + MQ 监听底座
4. 致命细节
   1. 兜底策略绝对不能丢
   2. 警惕“广播风暴”:高频修改的数据，绝对不适合放进 JVM 本地缓存！

## 内存删除策略 LRU 和LFU是怎么实现的

## 布隆过滤器

1. 定义:

   - **如果它说某个元素“不存在”，那么这个元素一定不存在。**

   - **如果它说某个元素“存在”，那么这个元素可能存在，也可能不存在（即存在“假阳性” False Positive）。**
