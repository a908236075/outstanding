...
6. 监视与通知
   ...
   2. 每次通知都是触发了一个监视点.
7. ZooKeeper 采用了一种基于 **TCP 长连接的“推拉结合”** 模式：客户端向服务端注册监听（Pull 时的附带动作），服务端在数据发生变化时主动通知客户端（Push），客户端收到通知后再去获取最新数据（Pull）。
8. ![通过通知的方式进行znode的变化](.\picture\zookeeper\通过通知的方式进行znode的变化.png)
