...
1. **第一把锁：Broker 端的队列锁（分布式锁）。** 消费者在向 Broker 拉取消息前，必须先向 Broker 申请该 MessageQueue 的独占锁。只有拿到锁的消费者节点，才有资格消费这个队列。这保证了在消费组内，发生 Rebalance（重平衡）时，同一个队列绝对不会被两个消费者同时消费。

   **第二把锁：消费者本地的队列锁。** 消费者拿到消息后，内部会有多个线程池来处理。为了防止同一个队列的消息被多个业务线程并发执行，本地也会对这批消息对应的 `ProcessQueue` 加锁。

   **挂起重试机制（悬挂等待）：** 在普通的并发消费中，如果某条消息消费失败，返回 `RECONSUME_LATER`，消息会被发往重试队列，下一条消息继续被消费（这就会导致乱序）。但在顺序消费中，如果处理失败，返回的是 **`SUSPEND_CURRENT_QUEUE_A_MOMENT`（挂起当前队列片刻）**。此时，消费者会**暂停拉取和消费该队列的后续消息**，等待一会儿后原位重试该消息，直到成功为止。这宁愿牺牲掉可用性（阻塞后面的消息），也要死守顺序性。
